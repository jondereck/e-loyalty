    ...
    <>
      ...
      <main className="container section">
        <div className="grid two" style={{ alignItems: "center" }}>
          <PhoneFrame active="card">
            ...
            <a href="#qr" className="row" style={{ marginTop: 14 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                ...
                <div><strong>Show My Pass</strong><br /><span className="muted">Open QR</span></div>
              </div>
              <Maximize2 size={18} />
            </a>
          ...
          <section>
            <div className="eyebrow">Customer Card</div>
            <h2>{data.profile.fullName}</h2>
            <div className="grid two">
              <div className="card"><strong>{data.todayEligibility}</strong><p className="muted">{data.nextEligibleAt ? `Next eligible: ${formatDateTime(data.nextEligibleAt)}` : "Ready to earn at any active branch."}</p></div>
              <div className="card"><strong>Last visit</strong><p className="muted">{data.lastVisit ? `${data.lastVisit.branch.name} - ${formatDateTime(data.lastVisit.scannedAt)}` : "No approved visits yet."}</p></div>
            </div>
